    ...
    <div className="space-y-4">
      ...
      <div className="font-mono text-sm">
        ...
        <p className="mt-1.5 font-mono text-xs text-muted-foreground/70">
          {bat.chargeFull.toFixed(0)} mAh / {bat.chargeFullDesign.toFixed(0)}{" "}
          mAh design · {bat.cycleCount} cycles
        </p>
      </div>
      ...
      <div className="space-y-2 pt-1">
        <Row
          k="ac power"
          v={acOnline ? "connected" : "disconnected"}
          accent={acOnline}
        />
        {bat.powerWatts > 0 && (
        ...
        <Row
          ...
          v={
            bat.manufacturer
              ? `${bat.manufacturer} ${bat.model}`.trim()
              : bat.model
          }
        />
      </div>
...
export function PowerMetrics() {
  ...
  return (
    <section id="power" className="scroll-mt-20">
      ...
      <div className="panel">
        {error && (
          <div className="px-4 py-2.5 font-mono text-sm text-destructive">
            ERR: {error}
          </div>
        )}
        ...
        {!loading && !error && !hasBatteries && (
          <div className="px-4 py-4 font-mono text-sm text-muted-foreground">
            ...
            {data?.acOnline ? "connected" : "disconnected"}
          </div>
        )}

        {hasBatteries && data && (
          <div
            className={`grid grid-cols-1 divide-y divide-border ${data.batteries.length > 1 ? "md:grid-cols-2 md:divide-x md:divide-y-0" : ""}`}
          >
