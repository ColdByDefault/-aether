...
// Sample CPU usage over a short interval by comparing idle/total cpu times.
...
async function getDiskInfo(): Promise<DiskInfo[]> {
  try {
    // -P for POSIX output, -k for 1K blocks, exclude pseudo filesystems
    ...
    for (const line of lines) {
      ...
      // Only include real mounts and skip zero-size
      if (total === 0) continue;
      disks.push({
        total,
        used,
    ...
    // Sort by total size descending, keep top 4 to avoid clutter
